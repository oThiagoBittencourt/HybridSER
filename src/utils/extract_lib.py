...
def extract_features(audio_path, output_dir):
    ...
    - Delta-Delta MFCCs as an image (dd_mfcc.png) - KERNEL ONLY
    - Chromagram as an image (chromagram.png) - KERNEL ONLY
    ...
    try:
        ...
        save_spec_as_image(chromagram, target_dir / 'chromagram.png', sr, y_axis='chroma')
...
def parse_french_name(name):
    ...
    FRENCH_EMOTIONS = {
        'C': 'Anger', 'D': 'Disgust', 'J': 'Joy', 'N': 'Neutral',
    ...
    try:
        ...
        if emotion_code not in FRENCH_EMOTIONS: return None
...
def parse_english_name(name):
    ...
    try:
        ...
        if emotion_code not in ENGLISH_EMOTIONS: return None
...
def parse_portuguese_name(name):
    ...
    PORTUGUESE_EMOTIONS = {
        'ale': 'Joy', 'des': 'Disgust', 'tri': 'Sadness', 'sur': 'Surprise',
        'rai': 'Anger', 'neu': 'Neutral', 'med': 'Fear'
    }
    try:
        parts = name.split('-')
        if len(parts) < 2: return None
        emotion_code = parts[0].lower()
        if emotion_code not in PORTUGUESE_EMOTIONS: return None
        emotion = PORTUGUESE_EMOTIONS[emotion_code]
        gender_code = parts[1][0].lower()
        if gender_code == 'f': gender = 'F'
        elif gender_code == 'm': gender = 'M'
        else: return None
